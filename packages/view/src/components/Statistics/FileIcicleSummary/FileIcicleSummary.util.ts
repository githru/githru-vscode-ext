...
import type { FileChanges, FileChangesMap, FileChangesNode, FileScoresMap } from "./FileIcicleSummary.type";
  ...
  return data
    ...
    .reduce(
      (map, { commit: { diffStatistics } }) =>
        Object.entries(diffStatistics.files).reduce((acc, [path, { insertions, deletions }]) => {
          acc[path] = {
...
const visitFileNode = (node: FileChangesNode[], path: string, fileChanges: FileChanges, score: number) => {
