...
1.명확성 : e2e 테스트는 특히나 더 명확해야 합니다. 실제 사용자 시나리오를 따르므로, 이해하기 쉽고 의미 있는 이름과 주석을 사용해야 합니다.

```text
테스트 코드는 input과 output에 집중하는 테스트가 목적이지 로직을 구현하는게 목표가 아닙니다. 코드가 복잡할 이유가 없습니다. 
```

2.독립성 : 각 테스트 케이스는 다른 테스트 케이스의 결과에 영향을 주지 않아야 합니다. 초기 상태를 설정하고 테스트 후에 상태를 되돌리는 것이 중요할 수 있습니다.
