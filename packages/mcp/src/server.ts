...
import type {
  FeatureImpactAnalyzerInputs,
  ContributorRecommenderInputs,
  ReactComponentTestInputs,
...
server.registerTool(
  ...
  async () => {
    ...
    try {
      ...
      if (!response.ok) {
        return {
          content: [{ type: "text", text: `Failed to fetch image: HTTP ${response.status} ${response.statusText}` }],
        };
      }
...
server.registerTool(
  "contributor_recommender",
  {
    ...
    description:
      "Recommends contributors who have contributed most to specific files/branches/PR areas by aggregating recent contribution data.",
    inputSchema: {
      ...
      chart: z
        .boolean()
        .default(false)
        .describe("Generate interactive chart visualization (true: HTML chart display, false: JSON data)"),
    },
  },
  ...
  async ({
    ...
    locale,
    chart,
  }: ContributorRecommenderInputs & { locale?: string; chart?: boolean }) => {
    try {
      I18n.setLocale(locale || "en");
      ...
      return {
        content: [{ type: "text", text: `Contributor recommendation error occurred: ${err?.message ?? String(err)}` }],
      };
    }
  }
);
...
server.registerTool(
  "generate_csm_dict",
  {
    ...
    inputSchema: {
      ...
      baseBranchName: z
        .string()
        .optional()
        .describe("Base branch name to analyze (default: repository's default branch)"),
      locale: z.enum(["en", "ko"]).default("en").describe("Response language (en: English, ko: Korean)"),
  ...
  async ({
    ...
    try {
      ...
      return {
        content: [{ type: "text", text: `CSM Dictionary generation error: ${err?.message ?? String(err)}` }],
      };
    }
  }
);
...
server.registerTool(
  "react_component_test",
  {
    ...
    inputSchema: {
      complexity: z
        .enum(["simple", "medium", "complex", "all"])
        .default("simple")
        .describe("Complexity level of React components to test"),
      componentType: z
        .enum(["basic", "chart", "form", "data-display", "interactive"])
        .optional()
  ...
  async ({ complexity, componentType }: ReactComponentTestInputs) => {
    try {
      ...
      return {
        content: [{ type: "text", text: `React component test error: ${err?.message ?? String(err)}` }],
      };
    }
  }
);
...
server.registerTool(
  "data_driven_component_test",
  {
    ...
    description:
      "Provides React components that work with data props to test Claude's understanding of data-driven patterns",
    inputSchema: {
      dataType: z
        .enum(["chart", "table", "list", "card", "all"])
        .default("all")
        .describe("Type of data-driven component to test"),
  ...
  async ({ dataType, sampleData }: DataDrivenComponentInputs) => {
    try {
      ...
      return {
        content: [{ type: "text", text: `Data-driven component test error: ${err?.message ?? String(err)}` }],
      };
    }
  }
);
...
server.registerTool(
  ...
  async ({ repo, githubToken, baseBranchName, locale, debug }) => {
    try {
      ...
      return {
        content: [{ type: "text", text: `Storyline UI rendering error: ${err?.message ?? String(err)}` }],
      };
    }
  }
);

// 📊 Author Workload + Commit Type Mix
server.registerTool(
  "author_work_pattern",
  {
