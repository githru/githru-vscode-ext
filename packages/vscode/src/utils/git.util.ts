...
export async function getBaseBranchName(path: string, repo: string): Promise<string> {
    return new Promise((resolve, reject) => {
        const args = ["symbolic-ref", "refs/remotes/origin/HEAD"];
        resolveSpawnOutput(cp.spawn(path, args, { cwd: repo, env: { ...process.env } })).then((values) => {
            const status = values[0];
            const stdout = values[1];
            const stderr = values[2];
            if (status.code === 0) {
                const remoteHeadBranchPaths = stdout.toString().trim().split("/");
                resolve(remoteHeadBranchPaths[remoteHeadBranchPaths.length - 1]);
            } else {
                reject(stderr);
            }
        });
